...
type Props = {
  ...
  compact?: boolean;
  /** When set (e.g. post detail), replaces post.commentsCount so the badge matches the comments API. */
...
export default function PostCard({
  ...
  // Portal-rendering with fixed coords lets the menu escape that.
  const recalcMenuPos = () => {
  ...
  // Close the kebab menu on outside click + ESC. Both the trigger AND the
  // portal'd menu count as "inside".
  useEffect(() => {
    if (!menuOpen) return;
  ...
  return (
    <>
        ...
        <div className="flex gap-0">
          ...
          <div className="flex-1 min-w-0 p-3.5">
            ...
            <div className="flex items-start gap-2 mb-2">
              ...
              {/* Kebab is shown to the owner (Edit/Delete) AND to any
                  other signed-in user (Report). Anonymous visitors don't
                  see it — there's nothing they can do from it. */}
              {(isOwner || (currentUserId && !isOwner)) && (
                <button
                  ...
                  className="ml-auto -mr-1 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-[var(--color-muted)] hover:bg-[var(--color-pill-bg)] hover:text-[var(--color-text)] transition-colors"
                >
      ...
      {/* Owner / report menu — portal'd to <body> so the card's
          `overflow-hidden` corner-rounding doesn't clip it.
          - Owner sees Edit + Delete.
          - Anyone else (signed in, not the author) sees Report. */}
      {menuOpen && menuPos && typeof document !== "undefined" && createPortal(
        ...
        >
          {isOwner && (
            <>
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-semibold text-[var(--color-text)] hover:bg-[var(--color-hover)]"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
          ...
          {!isOwner && currentUserId && (
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 3v18M3 3h13.5l-1.5 4 1.5 4H3" />
              </svg>
              Report
            </button>
          )}
